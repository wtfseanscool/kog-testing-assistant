...
export class ScreenshotManager {
  ...
  async addFromDataUrl(
    ...
    this.#updateSliderFill(sizeInput);
  ...
  #updateSliderFill(slider) {
    ...
    slider.style.background = `linear-gradient(to right, var(--accent) 0%, var(--accent) ${percent}%, var(--line) ${percent}%, var(--line) 100%)`;
  }
  ...
  #attachCardEvents(item) {
    ...
    item.sizeInput.addEventListener("input", () => {
      ...
      this.#updateSliderFill(item.sizeInput);
      this.#updateCanvasCursor(item);
      this.notifyChange();
    });
